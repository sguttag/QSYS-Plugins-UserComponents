...
- **RSTPoll_Interval**: (requires 1.3.1 or later) The rate that "Request Status (RST) command is issued.  This functions as a combination of keep-alive (must be issued every 60-seconds or less) as well as it is the only means that updates the "Segment Name" and Segment Index number in the plugin.  It defaults to 5-seconds to avoid an excessively laggy update to the Segment Name status.
...
- Q-SYS Designer 9.0 or higher (Primary development was on 9.13.1 LTS and checked on 9.4.8 LTS, 9.13.2 LTS, and 10.4.1)
- Q-SYS Core devices
